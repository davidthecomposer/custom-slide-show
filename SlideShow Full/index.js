...
const activePanelForward = () => {
	...
	const activeIndex = [...panes].findIndex((pane) =>
		pane.classList.value.includes("active-panel")
	);
	...
	if (panes[activeIndex] === panes[lastIndex]) {
		...
		slideText[lastIndex].classList.remove(
			slideRows[lastIndex].children[7].children[0].value
		);
		...
		panes[lastIndex].classList.remove(
			slideRows[lastIndex].children[8].children[0].value
		);
		panes[lastIndex].classList.add("invisible-panel");
		panes[0].classList.remove("invisible-panel");
		...
		panes[activeIndex + 1].setAttribute(
			"src",
			slideRows[activeIndex + 1].children[2].children[0].value
		);
		panes[activeIndex + 1].style.height =
			slideRows[activeIndex + 1].children[3].children[0].value;
		panes[activeIndex + 1].style.width =
			slideRows[activeIndex + 1].children[4].children[0].value;
		slideText[activeIndex].classList.remove(
			slideRows[activeIndex].children[7].children[0].value
		);
		slideText[activeIndex].classList.add("invisible-text");
		slideText[activeIndex + 1].classList.remove("invisible-text");
		slideText[activeIndex + 1].classList.add(
			slideRows[activeIndex + 1].children[7].children[0].value
		);
		slideText[activeIndex + 1].style.fontSize =
			slideRows[activeIndex + 1].children[5].children[0].value;
		slideText[activeIndex + 1].style.color =
			slideRows[activeIndex + 1].children[6].children[0].value;
		slideText[activeIndex + 1].innerHTML =
			slideRows[activeIndex + 1].children[9].children[0].value;
		...
		panes[activeIndex].classList.remove(
			slideRows[activeIndex].children[8].children[0].value
		);
		panes[activeIndex].classList.add("invisible-panel");
		panes[activeIndex + 1].classList.remove("invisible-panel");
		panes[activeIndex + 1].classList.add(
			slideRows[activeIndex + 1].children[8].children[0].value
		);
	}
};
...
const previousPane = () => {
	...
	if (panes[activeIndex] === panes[0]) {
		...
		panes[lastIndex].setAttribute(
			"src",
			slideRows[lastIndex].children[2].children[0].value
		);
		panes[lastIndex].style.height =
			slideRows[lastIndex].children[3].children[0].value;
		panes[lastIndex].style.width =
			slideRows[lastIndex].children[4].children[0].value;
		...
		slideText[lastIndex].classList.add(
			slideRows[lastIndex].children[7].children[0].value
		);
		slideText[lastIndex].style.fontSize =
			slideRows[lastIndex].children[5].children[0].value;
		slideText[lastIndex].style.color =
			slideRows[lastIndex].children[6].children[0].value;
		slideText[lastIndex].innerHTML =
			slideRows[lastIndex].children[9].children[0].value;
...
const autoPlayStart = () => {
	...
	if (
		apTimer.value !== "" &&
		apTimer.value.match(onlyNumbers) &&
		apTimer.value.match(onlyNumbers)[0].length === apTimer.value.length
	) {
		timerOnObject.timerOn = setInterval(
			activePanelForward,
			Number(apTimer.value)
		);
...
const changeMasterSize = () => {
	...
	if (
		masterHeight.value.match(validSize) &&
		masterWidth.value.match(validSize)
	) {
...
const initInputChecks = () => {
	const inputChecks = document.querySelectorAll(
		".image-height, .image-width, .font-size"
	);
...
const handleHints = (event) => {
	...
	if (tooltipToggle.getAttribute("src") === "images/toggleOn.svg") {
		if (classValue === "move-up") {
			...
			classValue.match("height") ||
			classValue.match("width")
		) {
...
const textAnimationOptions = () => {
	return animations.masterTextAnimations
		.map((ani) => `<option value="${ani}">${ani}</option>`)
		.join("");
};

//  This populates users slide animation options according to the master Array
const slideAnimationOptions = () => {
	return animations.masterSlideAnimations
		.map((ani) => `<option value="${ani}">${ani}</option>`)
		.join("");
};
...
const makeSlideRow = (image, mainImages) => {
	...
	newSlideRow.innerHTML = `
                ...
                <div class="inner-row-div i-height">
                   <input class="image-height"  type="text" size="8" value = "${Math.round(
											(image.height / [...panes][indexNumber].clientHeight) *
												100
										)}%"></input>
                </div>
                <div class="inner-row-div i-width">
                   <input class="image-width"  type="text" size="8" value = "${Math.round(
											(image.width / [...panes][indexNumber].clientWidth) * 100
										)}%"></input>
                </div>
                <div class="inner-row-div font-size-div">
                   <input class='font-size'  type="text" size="8" value = "${
											slideText[indexNumber].style.fontSize
										}"></input>
                </div>
                ...
                <div class="inner-row-div2 text-animation">
                   <select class="text-animation-styles" value="${
											animations.textAnimations[indexNumber]
										}">
                ...
                <div class="inner-row-div2 slide-animation" >
                   <select  class="slide-animation-styles" value="${
											animations.slideAnimations[indexNumber]
										}">
                   ${slideAnimationOptions()}
                </select>
                </div>
                <div class="inner-row-div2 text-content">
                <input class="text" type="text" size="8" value = "${
									slideText[indexNumber].innerHTML
								}"></input>
             </div>
...
`;
	...
	let slideAnimationStyles = document.querySelectorAll(
		".slide-animation-styles"
	);
	textAnimationStyles[indexNumber].value =
		animations.textAnimations[indexNumber];
	slideAnimationStyles[indexNumber].value =
		animations.slideAnimations[indexNumber];
	initInputChecks();
};
...
const handleFirstLast = () => {
	...
	if (slideRows.length > 0) {
		...
		[...slideRows][0].children[0].children[0].children[0].style.visibility =
			"hidden";
		[...slideRows][
			slideRows.length - 1
		].children[0].children[0].children[1].style.visibility = "hidden";
	}
};
...
`;
		...
		newSlideRow.children[0].children[0].children[1].addEventListener(
			"click",
			moveRow
		);
		newSlideRow.children[0].children[1].addEventListener("click", addNewRow);
		newSlideRow.children[0].children[2].addEventListener(
			"click",
			deleteSlideRow
		);
		newSlideRow
			.querySelectorAll(
				".move-up, .move-down, .add-row, .subtract-row, .image-src, .image-height, .image-width, .font-size"
			)
			.forEach((item) => {
	...
	const moveRow = (event) => {
		...
		if (event.target.classList.value === "move-up") {
			[...slideOptions.children][index - 1].insertAdjacentElement(
				"beforebegin",
				moveRowParent
			);
			[...paneContainer.children][index - 1].insertAdjacentElement(
				"beforebegin",
				paneToMove
			);
			[...textContainer.children][index - 1].insertAdjacentElement(
				"beforebegin",
				textToMove
			);
...
const createCodeDownload = () => {
	...
	const imageHeights = [...document.querySelectorAll(".image-height")].map(
		(img) => {
			return img.getAttribute("value");
		}
	);
	const imageWidths = [...document.querySelectorAll(".image-width")].map(
		(img) => {
			return img.getAttribute("value");
		}
	);
	...
	const fontColors = [...document.querySelectorAll(".input-color")].map(
		(img) => {
			return img.getAttribute("value");
		}
	);
	const textAnis = [...document.querySelectorAll(".text-animation-styles")].map(
		(img) => {
			return `'${img.getAttribute("value")}'`;
		}
	);
	const slideAnis = [
		...document.querySelectorAll(".slide-animation-styles"),
	].map((img) => {
		return `'${img.getAttribute("value")}'`;
	});
...
`;
    ...
    class SlideShow {
        ...
        autoPlayControl(event) {
            if (event === undefined) {
              this.timerOn = setInterval(() => {this.nextPane()}, Number(${
								apTimer.value
							}));
...
const eventListenersInit = () => {
	...
	const hints = document.querySelectorAll(
		".height, .width, .ap-timer, .move-up, .move-down, .add-row, .subtract-row, .image-src, .image-height, .image-width, .font-size"
	);
	const tooltipToggle = document.querySelector(".tooltip-toggle-button");
